...
class Olivia:
    ...
    def init(self) -> bool:
        ...
        try:
            print(f"[Olivia] Initializing in {self.mode.upper()} mode...")
            ...
            if self.mode == "socket":
                print("[Olivia] Connecting WebSocket...")
                if not self._connect_socket():
                    print("[Olivia] WebSocket connection failed, cannot continue in socket mode")
                    return False
                print("[Olivia] WebSocket connected successfully")
    ...
    def _setup_socket(self):
        ...
        def connect():
            ...
            # Check if we were authenticated before (reconnection case)
            with self._state_lock:
                was_authenticated = self._state == ConnectionState.AUTHENTICATED
            ...
            if had_session and self.session_id:
                if was_authenticated:
                    print("[Olivia] Reconnected to server")
        ...
        def disconnect():
            ...
            # Trigger reconnection if we had a session
            if had_session and self.session_id:
                print("[Olivia] Connection lost, reconnecting...")
        ...
        def on_heartbeat_ack(data):
            ...
            if isinstance(data, dict):
                ...
                if session_dead:
                    print(f"[Olivia] Server killed session: {reason}")
        ...
        def on_auth_response(data):
            ...
            if data.get('success'):
                print("[Olivia] Re-authenticated successfully")
                ...
                print(f"[Olivia] Re-authentication FAILED: {msg}")
        ...
        def on_session_expired(data=None):
            print(f"[Olivia] >>> RECEIVED session_expired event! Data: {data}")
            reason = "unknown"
            if isinstance(data, dict):
                reason = data.get('reason', data.get('message', 'unknown'))
            logger.warning(f"Server sent session_expired event: {reason}")
            print(f"[Olivia] Session terminated by server: {reason}")
            self.initialized = False
            self._handle_session_killed()
    ...
    def _handle_auth_response(self, response: Dict[str, Any]) -> bool:
        ...
        if self.mode == "socket":
            self._trigger_reconnect()  # Handles preventive refresh for Cloudflare
    ...
    def heartbeat(self) -> bool:
        ...
        if self.mode == "socket" and self._sio:
            ...
            # Socket.io's auto-reconnection handles disconnects (configured with infinite retries)
            ...
            if self.authenticated and self._server_public_key:
                try:
                    ...
                    if isinstance(response, dict):
                        ...
                        if not success:
                            ...
                            print(f"[Olivia] Heartbeat failed: {response.get('message', response.get('error', 'session invalid'))}")
                            self._handle_session_killed()
                            return False
                        return True
                ...
                try:
                    ...
                    if resp.status_code == 401:
                        ...
                        print("[Olivia] Session expired or deleted (HTTP 401)")
                        self._handle_session_killed()
                        return False
    ...
    # =========================================================================
    # UNIFIED RECONNECTION SYSTEM
    # =========================================================================

    def _trigger_reconnect(self):
        """Trigger reconnection manager (thread-safe, idempotent). Socket mode only."""
        # Only run in socket mode
        if self.mode != "socket":
            return
    ...
    def _reconnect_loop(self):
        """
        Unified reconnection loop handling:
        - Unexpected disconnects (with exponential backoff)
        - Preventive refresh (at 70s to avoid Cloudflare timeout)
        """
        while not self._reconnect_stop.is_set():
            ...
            if current_state == ConnectionState.AUTHENTICATED:
                # Check for preventive refresh (Cloudflare kills at ~100s)
                connection_age = time.time() - self._last_connect_time
                if connection_age >= 70:
                    logger.debug(f"Preventive refresh (age={connection_age:.0f}s)")
                    self._do_reconnect(preventive=True)
                else:
                    # Wait and check again
                    self._reconnect_stop.wait(10)
                continue
            ...
            if current_state == ConnectionState.DISCONNECTED:
                ...
                # Exponential backoff: 0.5, 1, 2, 4, 8, 16, max 30s
                delay = min(0.5 * (2 ** (self._reconnect_attempt - 1)), 30)
                print(f"[Olivia] Reconnecting... attempt {self._reconnect_attempt}/{self._max_reconnect_attempts}")

                if self._reconnect_stop.wait(delay):
                    return  # Stop requested

                self._do_reconnect(preventive=False)
            else:
                # CONNECTING or AUTHENTICATING - wait for state change
                self._reconnect_stop.wait(2)

    def _do_reconnect(self, preventive: bool = False):
        """Execute a single reconnection attempt."""
        reason = "preventive_refresh" if preventive else "reconnect_attempt"

        # For preventive refresh, we need to go from AUTHENTICATED -> CONNECTING
        # For unexpected disconnect, we're already DISCONNECTED
        with self._state_lock:
            current = self._state
            if current == ConnectionState.AUTHENTICATED and preventive:
                self._state = ConnectionState.CONNECTING
            elif current == ConnectionState.DISCONNECTED:
                self._state = ConnectionState.CONNECTING
            else:
                return  # Can't transition from current state
        ...
        try:
            ...
            # Success - connect event handler will update state and re-auth
    ...
    def _reauthenticate(self):
        ...
        if not self.session_id or not self._sio or not self._sio.connected:
            return

        self._transition_state(ConnectionState.AUTHENTICATING, "reauth_start")
        logger.debug("Re-authenticating session...")
    ...
    def _handle_session_killed(self):
        ...
        self._reconnect_stop.set()  # Stop reconnection attempts
