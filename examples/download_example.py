"""Authenticate first, then run your app's own download code."""
...
def main() -> int:
    ...
    try:
        ...
        if not session.has_subscription():
            print("Download blocked: no active subscription.", file=sys.stderr)
            return 1

        print("Authenticated. Start your download/update code here.")
        return 0
    finally:
        if api:
            api.close()
